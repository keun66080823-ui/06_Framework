...
-- 회원 1번 유저일 암호화된 비밀번호 업데이트 (평문은 pass01!)
UPDATE "MEMBER" SET
...
CREATE TABLE "BOARD" (
	...
	"BOARD_CONTENT"	VARCHAR2(4000)		NOT NULL, /* 상세 데이터(게시글 내용)*/
	"BOARD_WRITE_DATE"	DATE	DEFAULT SYSDATE	NOT NULL,
	"BOARD_UPDATE_DATE"	DATE		NULL,
	"READ_COUNT"	NUMBER	DEFAULT 0	NOT NULL,
	"BOARD_DEL_FL"	CHAR(1)	DEFAULT 'N'	NOT NULL,
	"BOARD_CODE"	NUMBER		NOT NULL, /* 공지 / 정보 / 자유 로 나눠 1,2,3만 들어올 수 있음*/
	"MEMBER_NO"	NUMBER		NOT NULL
);
...
CREATE TABLE "BOARD_IMG" (
	...
	"IMG_ORDER"	NUMBER		NULL, /* 이미지 오더 0번은 무조건 썸네일 이미지, 그 뒤 번호는 업로드 이미지별 순서 */
...
---------------------------------------------
/* 게시글 번호 시퀀스 생성 */
CREATE SEQUENCE SEQ_BOARD_NO NOCACHE; -- 게시글에 글번호 부여


BEGIN
	FOR I IN 1..2000 LOOP -- 1에서 2000까지 반복
		
		INSERT INTO "BOARD" -- 보더 테이블에 아래 내용을 넣겠다는 코드
		VALUES(SEQ_BOARD_NO.NEXTVAL,
					 SEQ_BOARD_NO.CURRVAL || '번째 게시글',
					 SEQ_BOARD_NO.CURRVAL || '번째 게시글 내용 입니다',
					 DEFAULT, DEFAULT, DEFAULT, DEFAULT,
					 CEIL( DBMS_RANDOM.VALUE(0,3) ), -- 보더 코드를 이렇게 쓴 이유는 0.0이상 3.0 미만의 난수에서 CEIL(올림)하면 1, 2, 3이 나오기 때문이다
					 1 -- 회원번호
		);
...
VALUES( SEQ_COMMENT_NO.NEXTVAL, '부모 댓글 1',
			  DEFAULT, DEFAULT,	2001, 1, NULL); -- 마지막의 NULL은 부모댓글의 유무로, null의 경우 부모 없는 1레벨 댓글
...
COMMENT ON COLUMN "MESSAGE"."CHATTING_ROOM_NO" IS '채팅방 번호';
	...
	SELECT CHATTING_ROOM_NO
		,(SELECT MESSAGE_CONTENT FROM 
			...
			WHERE ROWNUM = 1) LAST_MESSAGE -- 가장 마지막 메세지가 보내진 시간
	...
	WHERE OPEN_MEMBER = 1 -- 채팅방장으로 있거나
	OR PARTICIPANT = 1 -- 일반 사용자로 있거나
...
-- 채팅방 번호 체크
SELECT NVL(CHATTING_ROOM_NO, 0) CHATTING_ROOM_NO
FROM CHATTING_ROOM
