...
checkAuthKeyBtn.addEventListener("click", () => {
    ...
    fetch("/email/checkAuthKey", {
        method: "POST",
        headers: { "Content-Type": "application/json" }, // 요청보낼 데이터가 json형식
        body : JSON.stringify(obj)
    })
    .then(resp => resp.text()) // 응답 객체를 text로 변환
    .then(result => {
        // 1 or 0

        if(result == 0){
            authKeyMessage.innerText = "인증번호가 일치하지 않습니다";
            checkObj.authKey = false; // 메일 재인증 시도를 위해서 false로 바꿔야 함.
            return;
        }
...
memberNickname.addEventListener("input", (e) => {
    ...
    .then(count => {
        ...
        if(count == 1) { // 중복이다.
            ...
            nickMessage.classList.add("error"); // P.S. css파일에 만들어 둔 클래스다. 절대 내장 css가 아님.
            nickMessage.classList.remove("confirm");
            checkObj.memberNickname = false;
            return;
        }
