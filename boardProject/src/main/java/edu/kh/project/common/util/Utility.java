...
public class Utility {
	...
	public static int seqNum = 1; // 1 ~ 99999 번까지 1씩 증가하며 반복. 99999 초과 시 다시 1.
	
	public static String fileRename(String originalFileName) {
		// 20251211100330_00001.jpg // 2025-12-11 10시 3분 30초_동시간에만들어진파일의 순서.jpg
		
		// SimpleDateFormat : 시간을 원하는 형태의 문자열로 간단히 변경
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss"); // yyyyMMddHHmmss는 정해진 포맷이라 반드시 맞춰야 함.
		
		// java.util.Date() : 현재 시간을 저장한 자바 객체
		String date = sdf.format(new Date());
		
		String number = String.format("%05d", seqNum); // seqNum이 1이면 00001, 2면 00002로 이어짐.
