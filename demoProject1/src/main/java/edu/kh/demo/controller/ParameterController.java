...
@Controller // Controller는 요청/응답 제어하는 역할 명시 + Bean 등록을 한다.
@RequestMapping("param")	//	/param으로 시작하는 요청을 현재 컨트롤러로 매핑
@Slf4j	// log를 이용한 메시지를 콘솔창에 출력할 때 사용한다. (lombok 제공)
public class ParameterController {
	...
	// - 같은 name속성값을 가진 파라미터 얻어오기(배열 형태 String[], 리스트 형태 List<String>)
	...
	// - DTO에 setter가 필수적으로 존재해야 함
	
	// @ModelAttribute를 이용해 값이 필드에 세팅된 객체(아래 Test4에서는 inputMember)를
	// "커맨드 객체"라고 부름
	...
	public String paramTest4(/*@ModelAttribute*/ Member inputMember) {
		...
		// /param/main로 쓰면 절대경로 작성법이지만 앞에 /가 없으면 상대경로 작성법이다.
		// 상대경로에서 중요한 부분 ! -> 현재위치!!!
